...
export function BetPhase({ state, onSetBet, onStart }: Props) {
  ...
  const maxBet = state.cash;
  const targetScore = Math.round(cfg.target * (state.activeEventCard === 'high_roller' ? 1.5 : 1));

  // Estimate min payout (at exactly target score)
  const minCashoutMult = state.activeEventCard === 'greed_mode' ? 3.0 :
                         state.activeEventCard === 'high_roller' ? 5.0 : 2.0;
  const estMinPayout = Math.floor(state.bet * minCashoutMult);
    ...
    <div className="min-h-screen flex flex-col items-center justify-center gap-6 p-6">
      <div className="text-center">
        ...
        <div className="font-mono text-xs mt-1" style={{ color: 'var(--text-muted)', letterSpacing: '0.15em' }}>
          ROUND {state.round} / 6 · {cfg.isBoss ? '👑 BOSS ROUND' : `${cfg.bombs} BOMBS`}
        </div>
      </div>
      ...
      <div className="flex gap-6 font-mono text-sm">
        <div className="text-center">
          ...
          <div style={{ color: 'var(--text-primary)' }}>{targetScore.toLocaleString()} pts</div>
        </div>
        <div className="text-center">
        ...
        <div className="text-center">
          <div style={{ color: 'var(--text-muted)', fontSize: '0.7rem', letterSpacing: '0.15em' }}>LIVES</div>
          <div style={{ color: 'var(--red)' }}>{'❤️'.repeat(state.lives)}</div>
        </div>
      </div>
      ...
      <div className="casino-panel p-6 w-full max-w-sm flex flex-col gap-4">
        <div className="flex justify-between items-end">
          ...
          <div className="text-right">
            <div className="font-mono text-xs" style={{ color: 'var(--text-muted)', letterSpacing: '0.15em' }}>MIN PAYOUT</div>
            <div className="font-mono text-xl" style={{ color: 'var(--green)' }}>${estMinPayout}</div>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          {[0.25, 0.5, 1.0].map(pct => {
            const amt = Math.floor(maxBet * pct / 5) * 5;
            return (
              <button
                key={pct}
                onClick={() => onSetBet(Math.max(minBet, amt))}
